...
\begin{corollary}[Tseitin]\label{thm:tseitin}
  Every formula is equi-satisfiable to one in \textbf{Conjunctive Normal Form}.
\end{corollary}
...
While the successor function $+1$ is definable in terms of $<$, the opposite is not, \ie, the relation $<$ is only second-order definable in terms of $+1$.
Therefore, the first-order fragment of \SA with the ordering relation $<$ is strictly more expressive than with the successor function $+1$.
...
In fact, a state $s'$ is a successor of $s$ via $\t$ if $\rho_\t(V,V')$ evaluates to \true, \ie, $\tuple{s,s'} \models \rho_\t(V,V')$.
...
\end{center}
and, in this case, the tree in it is defined as $t: \text{dom}(t) \to A$, where $\text{dom}(t)$ is its set of nodes.
...
Any infinite word $\s = \tuplev{\s_0, \s_1, \ldots}$, where $\s_i \subseteq \AP$, is called a \textbf{trace} and contains the atomic propositions ($\s_i$) that should hold in that state.
...
\begin{note}
  Notice that $GF(\text{enabled} \to \text{taken})$ is not equal to the above: it is in fact strictly \emph{weaker} (it holds whenever the transition is infinitely often \emph{disabled}). The strictly stronger property is $G(\text{enabled} \to \text{taken})$, requiring the transition to be taken in every step in which it is enabled.
\end{note}
...
\begin{definition}[$\p$-atom]\label{def:p_atom}
  ...
  \begin{itemize}
    \item the conjunction of the sub-formulas in the atom is satisfiable, \ie, they can all hold together;
...
This allows model checking to be computed in polynomial time at the expense of losing expressiveness.
...
\begin{example*}[Adequate subgraph]
  ...
  Because it's both fair (due to being just) and fulfilling, by definition of an Adequate subgraph, the subgraph $S = \set{(s_0, A_1),(s_1, A_1),(s_2, A_1),(s_3, A_0)}$ is adequate, therefore it follows that LOOP has a computation that satisfies $\p \colon \msquare\Diamond(x = 3)$, \ie, $\p$ is P-satisfiable.
\end{example*}
...
\begin{definition*}[Tarski]
  ...
  \begin{itemize}
    \item $\bigcup$-continuous, then it holds that $\mu Z . \t Z = \bigcup_i \t^i(\false)$, \ie, we can reach the fixpoint going upwards, starting from the empty-set at the bottom, in a finite number of steps (it cannot increase forever), which is the typical construction for \emph{eventualities} ($\FF$/$\UU$);
    \item $\bigcap$-continuous, then it holds that $\nu Z . \t Z = \bigcap_i \t^i(\true)$, \ie, we can reach the fixpoint starting from $S$ and going downwards in a finite number of steps (it cannot decrease forever), which is the typical construction for \emph{safety} operators ($\GG$/$\RRR$).
  \end{itemize}
\end{definition*}
...
\begin{note}
  In the lasso case the successor $\succ(i)$ wraps around the loop, so the $\UU$ and $\FF$ equations are circular over the loop positions $l,\dots,k$ (the base case $\kloop{\p}^{k+1}=\bot$ is reached only in the loop-free case). They must therefore be read as \emph{least} fixpoints (and $\GG$/$\RRR$ as greatest ones); otherwise the all-true solution would spuriously satisfy, \eg, $\kloopil{\FF\p}$ on a loop where $\p$ never holds.
\end{note}
  ...
  \item The learner has two types of queries that he can ask:
    \begin{itemize}
      ...
      \item \emph{equivalence} query: check if the language $L$ that we're learning is equal to teacher's language $L_0$.
        Here, the teacher answers with a counter example (contradiction) of a word $w$ that belongs to $L$ but not to $L_0$, or vice versa, to prove that they're not equal.

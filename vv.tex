...
First, we encode the alphabet $A = \set{a,b}$ by mapping $a$ into $1$, and $b$ into $0$, like so:
\begin{center}
  \input{tikz/t0_encoding.tikz}
\end{center}
Then we create a second-order variable $X_1$ which captures the position of the nodes in the infinite path where the $1$ occurs, and a formula $\p (X_1)$ that ensures that there exists a path with infinitely many occurrences of an $a$, which is equal to:
\begin{equation*}
  ...
  \item The learner has two types of queries that he can ask:
    \begin{itemize}
      ...
      \item \emph{equivalence} query: check if the language $L$ that we're learning is equal to teacher's language $L_0$.
        Here, the teacher answers with a counter example: a word $w$ in the symmetric difference of $L$ and $L_0$ (accepted by one but not by the other), proving that they're not equal.

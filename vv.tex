...
\begin{definition*}[Reduction]
  A \textbf{reduction} $\leq$ from a problem $P$ to another $Q$ it's algorithm $F$ that solves $Q$ using a solution for $P$ as oracle:
  \begin{equation*}
...
\begin{lemma*}[Renaming]
  $\exists p\ \p$ is equivalent to $\exists p\ \p[p/q]$, provided $q$ doesn't occur free in $\p$.
\end{lemma*}
...
\begin{definition*}[Non-deterministic Finite state Automata (NFA)]
  ...
  \begin{itemize}
    ...
    \item $\D$ is a transition function \st $\D: Q \times A \to 2^Q$, where $2^Q$ is the \textbf{power set} of $Q$, \ie, the set of all the subsets of Q, or equivalently is expressed as $\D \subseteq Q \times A \times Q$. The meaning of this function is that given a symbol, we can reach more than one final states.
  \end{itemize}
\end{definition*}
...
\begin{definition*}[Relation $\approx_{\A}$]
  Let $\A = \NBA$ be a BA.
  We define a relation $\approx_{\A}$ on $A^*$ \st $\approx_{\A_Q^V}$ if for all pairs of state $s, s' \in Q$
  \begin{equation*}
...
\begin{example*}
  ...
  \begin{align*}
    \forall x,y\ [&x \in Q_a \land y \in Q_a \land x<y \land \\
                  &\neg\exists z \left( x<z \land z<y \land z \in Q_a \right) \to \exists X \left( x \in X \land \forall z \left( z \in X \to z+1 \notin X \right) \land y \notin X \right) ]
  \end{align*}
\end{example*}
...
\begin{itemize}
  \item $\text{chain}(X)\!: \forall x \in X, y \in Y \quad x<y \lor y<x \lor x=y$, \ie, a set of nodes that are prefix ordered, \eg, $\set{0, 00, 001, 00100, 0010001}$;
...
A CTL $M$ (a Kripke structure) is a triple $(S,R,L)$ where:
\begin{itemize}
...
\begin{itemize}
  ...
  \item if $v$ is an internal node and $\text{var}(v) = x_i$, then
    \begin{equation*}
      f_v(x_1,\dots,x_n) = \left( \neg x_i \land f_{\low(i)}(x_1,\dots,x_n) \right) \lor \left( x_i \land f_{\high(i)}(x_1,\dots,x_n) \right)
    \end{equation*}
...
A monitor for an LTL formula $\p$ can be constructed as a \textbf{deterministic finite automaton} (DFA) over $2^{AP}$ (the set of propositional valuations) with three sinks: $\top$, $\bot$, and a set of non-sink states representing the undecided region. The construction follows the tableau method for LTL: states correspond to consistent sets of obligations, and transitions advance the obligations by one step. Once a sink is reached, the verdict is final.

...
\begin{corollary}[Tseitin]\label{thm:tseitin}
  Every formula is equi-satisfiable to one in \textbf{Conjunctive Normal Form}. The transformation introduces, for a chosen subformula $\psi$, a fresh variable $z$ and replaces $\psi$ by $z$ while conjoining a definitional clause:
  \begin{equation*}
    \p[\psi] \qquad \leadsto \qquad \p[z] \;\land\; (z \to \psi)
  \end{equation*}
\end{corollary}
...
\begin{proof*}[\nameref{thm:tseitin}]
  ...
  Note also that only the implication $z \to (\a \land \beta)$ is added, not the full biconditional $z \leftrightarrow (\a \land \beta)$. This single direction suffices to preserve satisfiability precisely because $\p$ is in NNF, so the replaced subformula $\a \land \beta$ occurs with \emph{positive} polarity (the Plaisted--Greenbaum optimization). For a subformula occurring under a negation, the converse implication would be required instead.
\end{proof*}
...
\begin{lemma*}
  Model-checking QBFs in PNF with $n$ quantifier alterations is in $\sum_n = \exists^P \forall^P \ldots P$ or $\Pi_n = \forall^P \exists^P \ldots P$.
\end{lemma*}
...
\begin{example*}
  ...
  \begin{center}
    \input{tikz/buchi_example.tikz}
  \end{center}
  or equally
  \begin{center}
    \input{tikz/buchi_example2.tikz}
  \end{center}
...
\begin{definition}[Saturation]\label{def:saturation}
  ...
  \begin{equation*}
    U V^\w \cap L \neq 0 \quad \implies \quad U V^\w \subseteq L
  \end{equation*}
...
\begin{proof*}[\nameref{thm:comp_closure}]
  ...
  By the previous corollary, it follows that we can describe the complement of $L$ as $\bigcup_{i=1}^n U_i V_i^\w$, where $U_i,V_i$ are $\approx_{\A}$ classes \st $U_i V_i \cap \ol{L} \neq \emptyset$ for all $i$.
...
\begin{proof*}[1]
  \begin{itemize}
    \item[\color{pink!200}$\Rightarrow$] \ie, we want to prove that $U V^\w \subseteq U \overrightarrow{V}$ with $V V \subseteq V$.
      ...
      \begin{center}
        \input{tikz/lemma11_proof_a.tikz}
      \end{center}
...
\begin{proof*}[2]
  \begin{itemize}
    ...
    \item[\color{pink!200}$\Leftarrow$] \ie, we want to prove that $\overrightarrow{UV} \subseteq U \overrightarrow{V}$.
      ...
      \begin{center}
        \input{tikz/lemma11_proof_bb.tikz}
      \end{center}
...
While the successor function $+1$ is definable in terms of $<$, the opposite is not at the first-order level, \ie, the relation $<$ is only \emph{second-order} definable in terms of $+1$.
Therefore, the \emph{first-order} fragment with the ordering relation $<$ (\ie, $FO[<]$) is strictly more expressive than the one with only the successor function $+1$ ($FO[+1]$) -- which is precisely the gap relevant to star-free languages here. In full \SA, by contrast, $<$ and $+1$ are second-order interdefinable (see the redefinition of $<$ from $+1$ later in the S1S section), so there the two are equally expressive.
...
In fact, a state $s'$ is a successor of $s$ via $\t$ if $\rho_\t(V,V')$ evaluates to \true\ under the assignment interpreting the (unprimed) variables $V$ by $s$ and the primed variables $V'$ by $s'$, \ie, $\tuple{s,s'} \models \rho_\t(V,V')$.
...
To do so, we just need to change the representation of the symbols of our alphabet $A$ to $\set{0,1}^n$, where $n = \log_2 \abs{A}$.
    ...
    \item[\color{pink!200}$\Leftarrow$] 
      ...
      \begin{itemize}
        \item $X \subseteq Y$ \qquad \qquad \eg,
          ...
          \begin{center}
            \input{tikz/automata_subset.tikz}
          \end{center}
        \item $\succxy$

          And the automata that recognizes it is:
          \begin{center}
            \input{tikz/automata_suc.tikz}
          \end{center}
      \end{itemize}
...
\begin{center}
  \input{tikz/tree_example.tikz}
\end{center}
and, in this case, a tree in it is a labelling $t: \text{dom}(t) \to A$, where $\text{dom}(t)$ is its set of nodes (the domain of the tree).
...
\begin{exercise}\label{ex:tree_t0}
  ...
  \begin{center}
    \input{tikz/tree_exercise1.tikz}
  \end{center}
  ...
  \begin{center}
    \input{tikz/tree_exercise2.tikz}
  \end{center}
...
\begin{exercise*}
  ...
  \begin{center}
    \input{tikz/tree_exercise3.tikz}
  \end{center}
...
\begin{minipage}{0.4\textwidth}
  \begin{center}
    \input{tikz/modal_logic_ex.tikz}
  \end{center}
\end{minipage}
\begin{minipage}{0.4\textwidth}
  \begin{itemize}
...
\begin{center}
  \input{tikz/ltl_structure_ex.tikz}
\end{center}
...
\begin{itemize}
  ...
  \item \emph{branching}: each time point can have \textit{multiple successors}, thus multiple futures, and is used in CTL\footnote{Computation Tree Logic};
    \begin{center}
      \input{tikz/time_branching.tikz}
    \end{center}
...
Any infinite word $\s = \tuplev{\s_0, \s_1, \ldots}$, where each $\s_i \subseteq \AP$ is a \textbf{state} listing the atomic propositions that should hold at that position, is itself called a \textbf{trace} (or state sequence).
...
\begin{note}
  Notice that $GF(\text{enabled} \to \text{taken})$ is not equal to the above: it is in fact strictly \emph{weaker}. It is implied by $GF(\text{enabled}) \to GF(\text{taken})$ but not vice versa, because it is satisfied whenever the transition is infinitely often \emph{disabled} (then $\text{enabled} \to \text{taken}$ holds vacuously infinitely often), regardless of whether it is ever taken. The strictly \emph{stronger} requirement is instead $G(\text{enabled} \to \text{taken})$, \ie, the transition is taken in every step in which it is enabled.
\end{note}
...
\begin{definition}[$\p$-atom]\label{def:p_atom}
  ...
  \begin{itemize}
    \item the conjunction of the sub-formulas in the atom is satisfiable, \ie, all of them can hold simultaneously at one position of some model (the set is mutually consistent);
...
\begin{question*}[Is $\p = Gp \land F \neg p$ satisfiable?]
  ...
  \begin{center}
    \input{tikz/tableau_question.tikz}
  \end{center}
...
\begin{itemize}
  ...
  \item $R$ is the set of binary relations between states;
  \item $L$ is the set of propositional letters that holds for each state.
\end{itemize}
...
This allows model checking to be computed in polynomial time at the expense of losing expressiveness (in particular, as discussed below, CTL cannot express some fairness properties, although CTL model checking stays polynomial even when fairness constraints are imposed).
...
\begin{example*}
  ...
  \begin{center}
    \input{tikz/kripke_example.tikz}
  \end{center}
...
\begin{example}\label{exa:psi-sat}
  ...
  \begin{subfigure}{0.48\textwidth}
    \begin{center}
      \input{tikz/ex1_tableau.tikz}
    \end{center}
    \caption{Complete}
  \end{subfigure}
  \begin{subfigure}{0.48\textwidth}
    \begin{center}
      \input{tikz/ex1_tableau_pruned.tikz}
    \end{center}
    \caption{Pruned}
  \end{subfigure}
  ...
  \begin{center}
    \input{tikz/ex1_behavior_graph.tikz}
  \end{center}
...
\begin{example*}[Adequate subgraph]
  ...
  \begin{center}
    \input{tikz/ex2_tableau_pruned.tikz}
  \end{center}
  \begin{center}
    \input{tikz/ex2_behavior_graph.tikz}
  \end{center}
  ...
  Because it's both fair (due to being just) and fulfilling, by definition of an Adequate subgraph, the subgraph $S = \set{(s_0, A_1),(s_1, A_1),(s_2, A_1),(s_3, A_0)}$ is adequate, therefore it follows that LOOP has a computation that satisfies $\p \colon \msquare\Diamond(x = 3)$, \ie, $\p$ is P-satisfiable. Combined with the previous example, where $\neg\p$ has no adequate subgraph and is therefore \emph{not} P-satisfiable, this also shows that $\p$ is P-valid.
\end{example*}
...
\begin{example*}
  ...
  \begin{center}
    \input{tikz/ex4_behavior_graph.tikz}
  \end{center}
  \captionof*{figure}{Behavior graph $\B_{(\text{LOOP+},\psi)}$}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.48\textwidth}
    \begin{center}
      \input{tikz/decision_tree.tikz}
    \end{center}
  \end{subfigure}
  \begin{subfigure}{0.48\textwidth}
    \begin{center}
      \input{tikz/obdd.tikz}
    \end{center}
  \end{subfigure}
...
\begin{definition*}[Tarski]
  ...
  \begin{itemize}
    \item $\bigcup$-continuous, then it holds that $\mu Z . \t Z = \bigcup_i \t^i(\false)$, \ie, we can reach the fixpoint going upwards, starting from the empty-set at the bottom, in a finite number of steps (it cannot increase forever), which is the typical construction for \emph{eventualities} ($\FF$/$\UU$, \ie, $\EF, \AF, \EU, \AU$);
    \item $\bigcap$-continuous, then it holds that $\nu Z . \t Z = \bigcap_i \t^i(\true)$, \ie, we can reach the fixpoint starting from $S$ and going downwards in a finite number of steps (it cannot decrease forever), which is the typical construction for \emph{safety} operators ($\GG$/$\RRR$, \ie, $\AG, \EG$).
  \end{itemize}
\end{definition*}
...
\begin{note}
  In the lasso case the successor $\succ(i)$ wraps around the loop, so the equations for the eventualities $\UU$ and $\FF$ are \emph{circular} over the loop positions $l,\dots,k$ (the base case $\kloop{\p}^{k+1}=\bot$ is reached only in the loop-free unrolling). They must therefore be read as \emph{least} fixpoints: otherwise the all-true (greatest) solution would spuriously satisfy, \eg, $\kloopil{\FF\p}$ on a loop where $\p$ never holds. Dually, $\GG$ (and $\RRR$) are read as greatest fixpoints. Equivalently, one uses the explicit non-recursive unrolling that discharges each eventuality at some concrete position inside the loop.
\end{note}
...
\begin{example*}[Application of 'learning strategy']
  ...
  \begin{center}
    \input{tikz/learning_strategy_ex.tikz}
  \end{center}
  ...
  \begin{center}
    \input{tikz/learning_strategy_ex2.tikz}
  \end{center}

...
% --- styles for the diagrams reconstructed from the exam scribbles ---
\usetikzlibrary{arrows.meta,automata,positioning,calc}
\tikzset{
  bvar/.style ={circle,draw,minimum size=2.2em,inner sep=0pt},
  bleaf/.style={rectangle,draw,minimum size=1.9em,inner sep=2pt},
  blow/.style ={dashed,-{Stealth[]}},   % low  / 0 edge
  bhigh/.style={-{Stealth[]}},          % high / 1 edge
}
\newcommand{\bddlegend}{\par\smallskip{\footnotesize\itshape
  (dashed edge $=$ the $0$/low branch, solid edge $=$ the $1$/high branch)}}
...
\noindent The model drawn for Q2 (the lecture model-checking example): a
$4$-cycle with self-loops, $q_0$ initial and $a$ holding in $q_3$. The
automaton drawn for Q5 (to be complemented) is on the right.

\begin{center}
\input{tikz/exam_model_gfa.tikz}
\qquad\qquad
\input{tikz/exam_complement_inf_a.tikz}
\end{center}
...
\noindent A two-state BA for $(pq)^{\w}$ (treating $p,q$ as alphabet
symbols), with $q_0$ both initial and accepting:
\begin{center}
\input{tikz/exam_pq_omega.tikz}
\end{center}
...
\begin{center}
\input{tikz/exam_inf_many_a.tikz}
\qquad\qquad
\input{tikz/exam_fin_many_a.tikz}
\end{center}
...
clean fix is a nondeterministic initial choice: keep both automata and
add one fresh initial state (with no incoming edges) carrying the union
of the two old initial states' outgoing moves; the accepting set stays
$F_1\cup F_2$. This is the very point the examiner stressed in Exam~6,
Q4.
...
\[
  \text{justice (weak):}\quad FG(\mathit{enabled})\rightarrow GF(\mathit{taken}),
  \qquad
  \text{compassion (strong):}\quad GF(\mathit{enabled})\rightarrow GF(\mathit{taken}).
\]
The only difference is $FG$ vs.\ $GF$ in the antecedent: justice asks
only about transitions that are \emph{eventually permanently} enabled,
compassion about those enabled \emph{infinitely often}. Equivalently (the
negation forms used in the notes) justice is $\neg F\big(G(\mathit{enabled})\wedge
...
\subsection{The reduced OBDDs, drawn}

\paragraph{$(x\leftrightarrow y)\wedge R$ (Exam 1, Q5), order $x,y,R$.}
The two ``$y$ matches $x$'' branches feed a single shared $R$-node;
$R{=}1$ is the only way to reach the $1$-leaf.
\begin{center}
\input{tikz/exam_obdd_xy_r.tikz}
\bddlegend
\end{center}

\paragraph{$(x_1\leftrightarrow x_2)\wedge x_1$ (Exam 12, Q4), order $x_1,x_2$.}
The full binary decision tree (left) reduces --- merge the three equal
$0$-leaves, drop the redundant left $x_2$-node --- to the OBDD of
$x_1\wedge x_2$ (right):
\begin{center}
\input{tikz/exam_obdd_x1x2x1_tree.tikz}
\qquad\raisebox{1.1cm}{$\rightsquigarrow$}\qquad
\input{tikz/exam_obdd_x1x2x1_reduced.tikz}
\bddlegend
\end{center}

\paragraph{$(x_1\leftrightarrow x_2)\vee x_3$ (Exam 8, Q4), order $x_1,x_2,x_3$.}
When $x_1=x_2$ the answer is $1$ regardless of $x_3$; otherwise it is
$x_3$ (truth table $1,1,0,1,0,1,1,1$).
\begin{center}
\input{tikz/exam_obdd_x1x2x3.tikz}
\bddlegend
\end{center}
